...
inline void RenderCrosshair(ImDrawList* drawList)
{
    ...
    // Draw "Arcles External" text below crosshair
    if (Options::Crosshair::ShowText) {
        const char* Celex = "Arcles";
        const char* dotWin = " External";

        ImFont* font = ImGui::GetFont();
        float CelexW = font->CalcTextSizeA(font->FontSize, FLT_MAX, 0.0f, Celex).x;
        float dotWinW = font->CalcTextSizeA(font->FontSize, FLT_MAX, 0.0f, dotWin).x;
        float totalW = CelexW + dotWinW;
        ...
        // Draw outline for "Celex"
        for (int dx = -1; dx <= 1; ++dx) {
            for (int dy = -1; dy <= 1; ++dy) {
                if (dx || dy) {
                    drawList->AddText(ImVec2(textPos.x + dx, textPos.y + dy), outlineColor, Celex);
                }
            }
        }

        // Draw main text for "Celex"
        drawList->AddText(textPos, colWhite, Celex);

        // Draw outline for ".win"
        for (int dx = -1; dx <= 1; ++dx) {
            for (int dy = -1; dy <= 1; ++dy) {
                if (dx || dy) {
                    drawList->AddText(ImVec2(textPos.x + CelexW + dx, textPos.y + dy), outlineColor, dotWin);
                }
            }
        }

        // Draw main text for ".win"
        drawList->AddText(ImVec2(textPos.x + CelexW, textPos.y), colPink, dotWin);
    }
}
